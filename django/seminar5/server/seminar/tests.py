...
class SeminarFactory(DjangoModelFactory):
    ...
    name = factory.Faker('catch_phrase', locale='ko-KR')
    capacity = factory.LazyAttribute(lambda x: random.randint(1, 50))
    count = factory.LazyAttribute(lambda x: random.randint(1, 20))
    time = factory.LazyAttribute(timezone.now)
    online = factory.LazyAttribute(lambda x: bool(random.randint(0, 2)))
