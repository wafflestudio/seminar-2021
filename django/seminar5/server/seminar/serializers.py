...
class SeminarInstructorsSerializer(serializers.ModelSerializer):
...
class SeminarParticipantsSerializer(serializers.ModelSerializer):
...
class SeminarSerializer(serializers.ModelSerializer):
    ...
    def get_participants(self, instance):
        ...
        participants = SeminarParticipantsSerializer(instance.user_seminars.filter(is_instructor=False), many=True).data

        return participants

    def get_instructors(self, instance):

        instructors = SeminarInstructorsSerializer(instance.user_seminars.filter(is_instructor=True), many=True).data
    ...
    def update(self, instance, validated_data):
        ...
        if capacity is not None and instance.user_seminars.filter(is_active=True, is_instructor=False).count() > capacity:
            raise serializers.ValidationError(detail='이미 들어찬 정원보다 적게는 줄일 수 없어요', code='capacity error')
